...
class AppEngine {
  ...
  initDOMElements() {
    ...
    this.viewAllMemoriesBtn = document.getElementById('view-all-memories-btn');
    this.aboutTextContainer = document.getElementById('about-text-container');
    ...
    this.step2Badge = document.getElementById('step2-object-badge');
  ...
  initIntroFlow() {
    ...
    // Bind Start Button (Dismiss overlay)
  ...
  initDashboard() {
    ...
    if (this.editImageContainer) {
      this.editImageContainer.addEventListener('click', () => {
        ...
        if (obj) {
          this.newImageNameInput.value = obj.title;
          this.openModal(this.changeImageModal);
        }
      });
    }
    
    if (this.changeImageCloseBtn) {
      this.changeImageCloseBtn.addEventListener('click', () => {
        this.closeModal(this.changeImageModal);
      });
    }

    if (this.changeImageForm) {
      this.changeImageForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        if (!this.editingItem || this.editingItem.type !== 'object') return;
        ...
        this.changeImageLoading.classList.remove('hidden-modal');
        const submitBtn = document.getElementById('change-image-submit-btn');
        ...
        try {
          ...
          this.changeImageLoading.classList.add('hidden-modal');
      ...
      <div class="intro-demo-wrapper">
        ...
        <div class="intro-demo-memory fade-in-delayed">
          <div class="intro-demo-memory-header">
            ...
            <span class="intro-demo-close-mock" style="color: #999; font-weight: 300;">✕</span>
          </div>
          <div class="intro-demo-memory-text">
            ${displayMemory.text}
          </div>
          <div class="intro-demo-memory-author">- ${displayMemory.author}, ${displayMemory.location}</div>
          <div class="intro-demo-memory-date">${displayMemory.date}</div>
        </div>
      </div>
    `;
  ...
  selectObject(id) {
    ...
    // Center camera on object for premium feel (shift slightly left of center to balance drawer on desktop)
    const isMobile = window.innerWidth < 600;
    const centerOffset = isMobile ? 0 : 250;
    this.centerCameraOn(obj.x + centerOffset, obj.y, isMobile ? 0.65 : 1.1);
  }
  ...
  renderDrawerTabsContent(obj) {
    ...
    sortedMemories.forEach(mem => {
      ...
      memoryCard.innerHTML = `
        ...
        <div class="memory-meta-row">
          ...
          <button class="memory-like-btn ${likedClass}" data-mem-id="${mem.id}" aria-label="Like memory, current likes: ${mem.likes}">
            <svg viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>
            <span class="like-count">${mem.likes}</span>
          </button>
        </div>
      `;
      
      // Bind like click
      const likeBtn = memoryCard.querySelector('.memory-like-btn');
      likeBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        this.toggleLikeMemory(obj.id, mem.id, likeBtn);
      });
    ...
    // 2. About Tab
    this.aboutTextContainer.innerHTML = obj.about || '<p>No details logged for this object yet.</p>';
    
    // 3. In Numbers Tab
    this.numbersContainer.innerHTML = '';
    if (obj.stats && obj.stats.length > 0) {
      obj.stats.forEach(stat => {
        const card = document.createElement('div');
        card.className = 'crisis-stat-card';
        card.innerHTML = `
          <div class="crisis-stat-header">
            <span>Indicator</span>
            <span>2022 Crisis Profile</span>
          </div>
          <div class="crisis-stat-value">${stat.value}</div>
          <div class="crisis-stat-desc"><strong>${stat.label}</strong>: ${stat.desc}</div>
          <div class="crisis-progress-bar">
            <div class="crisis-progress-fill" style="width: 0%"></div>
          </div>
        `;
        this.numbersContainer.appendChild(card);
        
        // Trigger fill animation with a minor lag for premium smoothness
        setTimeout(() => {
          const fill = card.querySelector('.crisis-progress-fill');
          if (fill) fill.style.width = stat.value.includes('%') ? stat.value : '75%';
        }, 100);
      });
    } else {
      this.numbersContainer.innerHTML = '<p>Statistical crisis profiles are loaded dynamically.</p>';
    }
  }
  ...
  openModal(modalElement) {
    if (modalElement.id === 'add-object-modal') {
      ...
      if (this.floatingCalloutCard) this.floatingCalloutCard.classList.add('hidden-panel');
    } else {
  ...
  closeModal(modalElement) {
    ...
    if (modalElement.id === 'add-object-modal') {
      if (this.floatingCalloutCard) this.floatingCalloutCard.classList.remove('hidden-panel');
      // Reset steps after the close animation completes
      setTimeout(() => {
  ...
  // Reset two-step modal back to step 1
  resetModalSteps() {
    if (!this.modalStep1 || !this.modalStep2) return;
    this.modalStep1.classList.remove('step-exit');
    this.modalStep1.classList.remove('modal-step-hidden'); // This was missing
    this.modalStep2.classList.remove('step-enter');
    this.modalStep2.classList.add('modal-step-hidden');
    this.pendingNewObjectId = null;
  }
  ...
  async fetchOrGenerateImage(newTitle) {
    ...
    const promptText = [
      ...
      `Lighting: soft diffuse lighting, emotionally neutral. No dramatic shadows, no cinematic contrast, no glossy highlights, no rim lighting. Shading only to preserve form and clarify material.`,
      `Colors: muted, slightly desaturated, archival in tone. Materially believable, historically grounded. No hyper-commercial vibrancy.`,
      `Edges: slightly irregular, hand-drawn feel, organically imperfect. Not sterile, not vector-flat.`,
      `Composition: single centered object, no environment, no props, no narrative scene, no ground plane, no cast shadow on background. Object presented like a catalogue specimen or ethnographic artefact.`,
      `Camera: slightly elevated frontal perspective, approximately 20 degrees top-down angle. Object fully visible, top surfaces slightly revealed, clear silhouette.`,
      `Cultural context: everyday Sri Lankan domestic object, ordinary and familiar.`,
      `Background: absolutely pure flat white (#FFFFFF). No gradient, no vignette, no shadow, no texture on background. Only the object exists.`,
      `This is NOT: photorealism, vector art, cartoon, flat illustration, 3D render, isometric, infographic, or product photography.`
    ].join(' ');
  ...
  async generateCustomObject(userLabel) {
    ...
    if (overlay) overlay.classList.remove('hidden-modal');
    if (submitBtn) submitBtn.disabled = true;
    ...
    try {
      ...
      this.step2Badge.textContent = `${displayLabel} added to canvas`;
      ...
      this.modalStep1.classList.add('step-exit');
      setTimeout(() => {
        this.modalStep1.classList.add('modal-step-hidden');
        this.modalStep2.classList.remove('modal-step-hidden');
        this.modalStep2.classList.add('step-enter');
      }, 300);
      ...
      if (this.addObjectError) {
        ...
        this.addObjectError.innerHTML = `<strong>Error.</strong> Failed to fetch or generate object.`;
      }
      if (overlay) overlay.classList.add('hidden-modal');
      if (submitBtn) submitBtn.disabled = false;
      if (formInput) formInput.disabled = false;
  ...
  bindEvents() {
    ...
    this.viewport.addEventListener('mousedown', (e) => {
      ...
      if (e.target.closest('.floating-panel') || e.target.closest('#side-drawer') || e.target.closest('.modal-dialog')) return;
    ...
    this.addObjectForm.addEventListener('submit', (e) => {
      ...
      this.modalStep1.classList.remove('step-enter');
      this.modalStep1.classList.add('step-exit');
      setTimeout(() => {
        this.modalStep1.classList.add('modal-step-hidden');
        this.modalStep2.classList.remove('modal-step-hidden');
        ...
        const displayLabel = matchedTemplate.title;
        this.step2Badge.textContent = `${displayLabel} added to canvas`;
    ...
    this.addMemoryTrigger.addEventListener('click', () => {
      const handleOpenMemory = () => {
        const activeObj = this.objects.find(o => o.id === this.activeObjectId);
        if (!activeObj) return;
        if (this.memoryTargetName) {
          this.memoryTargetName.textContent = activeObj.title;
        }
        this.addMemoryForm.reset();
        this.openModal(this.addMemoryModal);
      };
    ...
    this.addMemoryForm.addEventListener('submit', (e) => {
      ...
      // Clean and close
      this.addMemoryForm.reset();
      this.closeModal(this.addMemoryModal);
    });
    ...
    window.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') {
        if (!this.addObjectModal.classList.contains('hidden-modal')) {
          this.closeModal(this.addObjectModal);
        } else if (!this.addMemoryModal.classList.contains('hidden-modal')) {
          this.closeModal(this.addMemoryModal);
        } else if (this.activeObjectId) {
          this.closeDrawer();
        }
